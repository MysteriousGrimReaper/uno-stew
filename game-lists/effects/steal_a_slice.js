	...
	async effect({ uno_players, player }) {
		if (uno_players.length == 2) {
			await uno_players.game_channel.send(
				uno_players.next_player.pizza == 0
					? `Your opponent has no more pizza, they must draw one card.`
					: `You stole a slice of pizza from your opponent.`
			);
		...
		const trade_hands_promise = new Promise((resolve) => {
			const filter = (m) =>
				...
				uno_players.find_player_by_name(m.content.toLowerCase()); // Only collect messages from the author of the command
			...
			collector.on("collect", async (collectedMessage) => {
				...
				if (target_player) {
					await collectedMessage.reply(
						target_player.pizza == 0
							? `That player has no more pizza, they will draw one card.`
							: `You stole a slice of pizza from ${target_player.user}.`
					);
